    ...
    <section id="productDetail">
      <div className="w_100 boxRow pdCard pt_2">
        ...
        <motion.div
          ...
          transition={{ duration: 0.6 }}
        >
          <h1 className="text_h1 w_50 selfLeft">{location.state.name}</h1>
          <p className="opacity_50 w_100">
            {location.state.desc
              ? location.state.desc
              : "The new XX99 Mark II headphones is the pinnacle of pristine audio. It redefines your premium headphone experience by reproducing the balanced depth and precision of studio-quality sound."}
          </p>
          <p className="text_body w_100">$ {randonNumber}</p>
          <div className="boxRow gap_1 left w_100 fleWrap">
            <div className="boxRow bgGray">
              <button className="p_1" onClick={handlerDecBtn}>
                -
              </button>
              <button className="p_1">{count}</button>
              <button className="p_1" onClick={handlerIncBtn}>
                +
              </button>
            </div>
            <button onClick={handlerCartAdd} className="btn_orange selfLeft">
              ADD TO CART
            </button>
          </div>
        </motion.div>
